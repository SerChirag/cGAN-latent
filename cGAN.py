...
feature_vectors = []
distance_separation = 0.8
with open('feature_vectors_ae.pkl', 'rb') as f:
    feature_vectors = pickle.load(f)

def get_latent(batch_input, num_of_samples = BATCH_SIZE):
    class_condition = batch_input.cpu().detach().numpy()
    ...
    noise = np.random.normal(0, np.sqrt(1.0/Z_DIM)*distance_separation , (num_of_samples,Z_DIM))
...
fixed_z = get_latent(fixed_label,NUM_CLASSES * IMGS_TO_DISPLAY_PER_CLASS)
